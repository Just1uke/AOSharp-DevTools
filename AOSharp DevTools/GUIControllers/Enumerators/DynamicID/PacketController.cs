...
namespace DevTools.GUIControllers.Enumerators.DynamicID
{
    public class PacketController : DynamicIDController
    {
        ...
        public const int GFX_MIN_VALUE = 0;
        public const int GFX_MAX_VALUE = 424;
        public PACKETController(String PluginDirectory, AOSharp.Core.Settings PluginSettings) : base(PluginDirectory, PluginSettings)
        {
        ...
        {
            return base.InitializeWindow(PluginDirectory + "\\UI\\Enumerators\\DynamicID\\PACKET.xml", new Rect(0, 0, 455, 345),
                windowName: "Developer Tools - PACKET Enumerator",
        ...
        {
            Window newWindow = Window.GetWindowByName("Developer Tools - PACKET Enumerator");
            if (newWindow != null && newWindow.IsValid)
            {
                View outView;
                newWindow.FindView("PACKET Enumerator", out outView);
                if (outView != null)
                {
                    if (outView.FindChild("GFXSelectSlider", out AOSharp.Core.UI.SliderView gfxSelectSlider))
                    {
                        ...
                        {
                            ChangeBigmapValue(outView, (int)gfxSelectSlider.GetValue());
                        }
                        if (outView.FindChild("brnDecreaseGFXId", out AOSharp.Core.UI.Button btnDecreaseGFXId))
                        {
                            ...
                            btnDecreaseGFXId.Clicked = (object s, ButtonBase button) => {
                                ChangeBigmapValue(outView, curSliderValue - 1);
                        ...
                        if (outView.FindChild("btnSetGFXId", out AOSharp.Core.UI.Button btnSetId))
                        {
                            ...
                            btnSetId.Clicked = (object s, ButtonBase button) => {
                                if (outView.FindChild("tivGFXId", out AOSharp.Core.UI.TextInputView idInput))
                                {
                                    ...
                                    {
                                        idInput.Text = $"{ChangeBigmapValue(outView, Int16.Parse(idInput.Text))}";
                                    }
        ...
        unsafe private int ChangeBigmapValue(View view, int value)
        {
            ...
            if (value <= GFX_MAX_VALUE && value >= GFX_MIN_VALUE)
            {
                try
                {
                    try
                    {
                        char* dynamicIdChar = DynamicID_t.GetID(DynamicID_t.GetInstance(), "PACKET_", value);
